...
* **vLLM**: `main`, pinned. The backend API this plugin implements
  ...
  `thunder_attn.py` reference only exist on `main`; the latest tag does not
...
vllm serve <model> --attention-backend CUSTOM \
  --kv-cache-dtype thunder_cute
# optional knobs (defaults shown)
export THUNDER_K_BITS=4
export THUNDER_V_BITS=4
```
...
slot; `ThunderCacheLayout.k_codes/v_codes/views` produce zero-copy views in
...
## vs thunder-vllm: expected performance
...
Generated by `benchmarks/bench_vs_thunder_vllm.py` into
`benchmarks/results/vs_thunder_vllm.md`.
...
2. **tcgen05/TMEM: sync fixed, operand K-order mismatch remains (path A).**
   `attention/cute_kernel_tcgen05.py` (opt-in via `THUNDER_SCHEDULE`)
...
10. **`kv_cache_dtype` is not a real vLLM preset.** vLLM's `CacheDType` is a
   ...
   `THUNDER_K_BITS`/`THUNDER_V_BITS` (or `ThunderCuteConfig`), while the
...
# 3. plugin vs upstream vLLM TurboQuant (writes .../vs_thunder_vllm.md)
python -m benchmarks.bench_vs_thunder_vllm \
  --model Qwen/Qwen3-4B --out benchmarks/results/vs_thunder_vllm.md

# 4. ablation (writes .../ablation.md)
python -m pytest tests/test_ablation.py -s

# 5. everything, with optional ncu profiling
THUNDER_KERNEL_ENABLE=1 bash benchmarks/run_all.sh
PROFILE=1 THUNDER_KERNEL_ENABLE=1 bash benchmarks/run_all.sh
```
...
├── thunder_vllm/
│   ├── attention/
│   │   ├── backend.py        # ThunderAttentionBackend + Impl
