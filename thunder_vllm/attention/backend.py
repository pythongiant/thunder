...
_STAGE = {"total": [], "prefix": [], "gather": [], "qrot": [], "launch": [],
          "inverse": [], "suffix": []}
# Host-dispatch buckets only: these perf_counter intervals do not synchronize
# CUDA, so they cannot measure GPU execution time. The normal vLLM-owned
# do_kv_cache_update store path also runs outside forward and is not included
# in _STAGE["total"].
...
def _dump_stage() -> None:
    ...
    _emit_telemetry("stage", {
        k: {
            "p50_ms": pct(v)[0], "p90_ms": pct(v)[1], "n": len(v),
            "min_ms": min(v) if v else 0.0, "max_ms": max(v) if v else 0.0,
            "values_ms": list(v),
        }
        for k, v in _STAGE.items()
    })
...
def _dump_fwd_count() -> None:
    if env_flag("THUNDER_COUNT"):
        ...
        from thunder_vllm.utils.telemetry import emit as _emit_telemetry
        _emit_telemetry("count", {
            "forward_calls": {"|".join(k): v for k, v in _FWD_COUNT.items()},
        })
...
class ThunderAttentionImpl(AttentionImplBase):
    ...
    def warmup(
        ...
        *,
        ...
        """Pre-compile the kernel and ensure gather buffers.
