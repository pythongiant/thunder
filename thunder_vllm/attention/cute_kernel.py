...
def _dump_counts() -> None:
    if _DIAG:
        ...
        _emit_telemetry("diag", {"counters": dict(_COUNTS)})
...
def _dump_launch_time() -> None:
    ...
    from thunder_vllm.utils.telemetry import emit as _emit_telemetry
    _emit_telemetry("launch", {"n": _LAUNCH_TIME["n"], "mean_ms": means})
    ...
    *,
    ...
    if _FASTLAUNCH and not torch.cuda.is_current_stream_capturing():
        ...
        key = _fast_key(
            ...
            num_reqs, max_query_len, S, debug, split_mode, gqa_mode, gqa_pack,
