...
HD, HK, BS = 128, 8, 16
# Query-head count for the bench. Default 8 preserves the old microbench;
# set THUNDER_BENCH_HQ=32 for the engine-like GQA shape (32 Q heads, 8 KV heads).
HQ = int(os.environ.get("THUNDER_BENCH_HQ", 8))
# GQA-packed decode schedule (plan steps 6+8). Decode-only; a no-op unless
# HQ > HK. GPU validation pending.
GQA_PACK = os.environ.get("THUNDER_GQA_PACK", "0").strip().lower() not in (
    "", "0", "false", "no", "off")
...
def bench(tag: str, k_bits: int, v_bits: int, nt: int, causal: bool) -> None:
    ...
    def call():
        launch_thunder_attention(fwd, qr, g, out, meta, HD ** -0.5, quantizer=quant,
                                 gqa_pack=GQA_PACK and not causal)
...
def main() -> int:
    print(f"device: {torch.cuda.get_device_name(0)} m_block={MB} n_block={NB}",
          flush=True)
    print(f"heads: Q={HQ} KV={HK} gqa_pack={GQA_PACK}", flush=True)
