...
def kernel_parity(k_bits: int, v_bits: int, causal: bool, nt: int = 256,
    ...
    ref = F.scaled_dot_product_attention(
        ...
        is_causal=causal, scale=HD ** -0.5,
        enable_gqa=gqa)[0].transpose(0, 1).float()
...
def main() -> int:
    ...
    # GQA-packed decode (plan steps 6+8). GPU validation pending.
    for k_bits, v_bits in ((4, 4), (3, 4)):
        kernel_parity(k_bits, v_bits, causal=False, flags=(True, True, True),
                      gqa=True)
